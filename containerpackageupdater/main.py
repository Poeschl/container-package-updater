...
def main(token: str, container_file: str, push_repository: str, os_version: str, architectures: list[str]) -> int:
  reset_to_main_branch()
  ...
  for package in packages:
    ...
    if len(latest_packages) == 1:
      ...
      if package.version != latest_package.version:
        update_single_version(package, latest_package, container_file, package_manager, container_file_content, push_repository, token)
      else:
...
if __name__ == '__main__':
  ...
  exit(main(args.token, args.containerFile, args.repository, args.osVersion, args.architecture))
